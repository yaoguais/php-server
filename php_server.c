...
PHP_INI_BEGIN()
    STD_PHP_INI_ENTRY("php_server.global_value",      "42", PHP_INI_ALL, OnUpdateLong, global_value, zend_php_server_globals, php_server_globals)
    STD_PHP_INI_ENTRY("php_server.global_string", "foobar", PHP_INI_ALL, OnUpdateString, global_string, zend_php_server_globals, php_server_globals)
PHP_INI_END()
*/
/* }}} */
...
/* {{{ proto string confirm_php_server_compiled(string arg)
   Return a string to confirm that the module is compiled in */
PHP_FUNCTION(confirm_php_server_compiled)
{
	char *arg = NULL;
	size_t arg_len, len;
	char *strg;
	...
	len = spprintf(&strg, 0, "Congratulations! You have successfully modified ext/%.78s/config.m4. Module %.78s is now compiled into PHP.", "php_server", arg);
...
{
	php_server_globals->global_value = 0;
	php_server_globals->global_string = NULL;
}
*/
/* }}} */
...
const zend_function_entry php_server_functions[] = {
	PHP_FE(confirm_php_server_compiled,	NULL)		/* For testing, remove later. */
